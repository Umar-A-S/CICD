        ...
        <form action="{{ route('superadmin.users.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="username" name="username" value="{{ old('username') }}" required
                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-lime-400"
                    placeholder="Contoh: adminmagelang">
                <p class="text-gray-500 text-xs mt-1">ℹ️ Username untuk login (tidak boleh sama dengan yang lain)</p>
                @error('username')
                    <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div id="wilayah-container" class="hidden">
                ...
                <select id="kode_wilayah" name="kode_wilayah"
                    ...
                    @foreach ($wilayahOptions as $kode => $nama)
                        <option value="{{ $kode }}" {{ old('kode_wilayah') === $kode ? 'selected' : '' }}>
                            {{ $nama }} ({{ $kode }})
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <p class="text-gray-500 text-xs mt-1">ℹ️ Minimal 6 karakter</p>
                @error('password')
                    <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
